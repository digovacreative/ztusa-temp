...
function list_searcheable_acf(){
  $list_searcheable_acf = array("content_fields", "event_content", "biography", "location", "phone", "career_highlights", "membership_details", "search_keywords");
            ...
            OR EXISTS (
              ...
              WHERE (
          		taxonomy = 'post_tag'
            		...
            		OR taxonomy = 'insights_type'
                OR taxonomy = 'people_role'
          		)
...
function advanced_custom_search( $where, &$wp_query ) {
    ...
    $where .= "
      AND (
        (".$wpdb->prefix."posts.post_title LIKE '%".$terms."%')
        OR (".$wpdb->prefix."posts.post_content LIKE '%".$terms."%')
        OR EXISTS (
          SELECT * FROM ".$wpdb->prefix."postmeta
            WHERE post_id = ".$wpdb->prefix."posts.ID
              AND (";
      ...
      $where .= ")
        OR EXISTS (
          ...
          taxonomy = 'post_tag'
            ...
            AND object_id = ".$wpdb->prefix."posts.ID
            AND ".$wpdb->prefix."terms.name LIKE '%".$terms."%'
        )
    )";
    echo '<pre>'.$where.'</pre>';
